...
export class TriggerScheduleController {
    ...
    formSchedule(): void {
        const now = new Date();
        const timedTriggers = triggersController.getTimedTriggers();
        const withoutPastTriggers: Trigger[] = timedTriggers.filter((trigger: Trigger) => {
        ...
        if (sorted.length === 0) {
            this.schedule = timedTriggers[timedTriggers.length - 1]
        } else {
